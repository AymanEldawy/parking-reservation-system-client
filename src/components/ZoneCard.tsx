...
const ZoneCard = ({ zone, setSelectedZone, selectedZone }: { zone: ZoneType, selectedZone: ZoneType | undefined, setSelectedZone: (zone: ZoneType) => void, gateId: string | undefined }) => {
  ...
  return (
    <Card onClick={handleSelectZone} className={`gap-2 group cursor-pointer rounded-lg border border-gray-200 bg-white shadow-sm transition-all hover:shadow-lg  ${selectedZone?.id === zone.id ? 'border-2 border-[var(--primary-color)]' : ''} `}>
      ...
      <CardContent className="py-0 px-4">
        ...
        <div className='flex-container items-center mt-4'>
          <div className="text-sm">
            <p className="text-[var(--text-secondary)] whitespace-nowrap">Rate Normal:  <span className='text-base font-bold'> ${zone.rateNormal}/hr</span></p>
            <p className="text-[var(--text-secondary)] whitespace-nowrap">Rate Special:  <span className='text-base font-bold'> ${zone.rateSpecial}/hr</span></p>
          </div>
        </div>
